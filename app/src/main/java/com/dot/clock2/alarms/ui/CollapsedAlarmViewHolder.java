...
import com.dot.clock2.alarms.misc.DaysOfWeek;
...
public class CollapsedAlarmViewHolder extends BaseAlarmViewHolder {
    ...
    @Bind(com.dot.clock2.R.id.countdown) AlarmCountdown mCountdown;
    @Bind(com.dot.clock2.R.id.recurring_days) TextView mDays; // TODO: use `new DateFormatSymbols().getShortWeekdays()` to set texts

    public CollapsedAlarmViewHolder(ViewGroup parent, OnListItemInteractionListener<Alarm> listener,
                                    AlarmController alarmController) {
        super(parent, com.dot.clock2.R.layout.item_collapsed_alarm, listener, alarmController);
    }
    ...
    private void bindDays(Alarm alarm) {
        ...
        if (num == DaysOfWeek.NUM_DAYS) {
            text = getContext().getString(com.dot.clock2.R.string.every_day);
        } else if (num == 0) {
            text = "";
        } else {
